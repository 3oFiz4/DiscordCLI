...
with open('conf.json', 'r', encoding='utf-8') as f:
    _ = json.load(f)
cmdK = _["cmdKey"]
commands_config = _["commands"] # New variable for easier access to commands configuration
...
class DiscordCompleter(Completer):
    ...
    def get_completions(self, document: Document, complete_event):
        ...
        if text.startswith(forward_msg_prefix):
            parts = text.split(" ", 2)
            if len(parts) < 3: return # User still typing for index param
            pref = parts[-1]
                ...
                if "{" in last and "}" not in last:
                    ...
                    for key in snippet_keys:
                        if key.startswith(prefix):
                            yield Completion(f"{key}}}", start_position=-len(prefix)-1)
    ...
    def fmt_author(self, member):
        ...
        if self.show_displayname and hasattr(member, "display_name"):
            if self.show_username:
                return _["events"]["format"]["author"].format(display_name=member.display_name, name=member.name)
            else:
                return member.name
        return member.name
# ... existing code ...
    ...
    async def on_message(self, message):
        ...
        if self.user in message.mentions:
            console.bell()
            console.print(_["events"]["onMessage"].format(author=message.author, channel=message.channel))
...
async def start_cli(client):
    ...
    def get_command_prefix_len(text_input, command_name):
        ...
        for alias in cmd_aliases:
            ...
            elif text_input == full_cmd: # This handles the case where no argument is given
                return len(full_cmd)
    ...
    while True:
        ...
        # Server Navigation
            ...
            if guild:
                ...
                console.print(commands_config["server_nav"]["logs"]["success"].format(name=guild.name))
            else:
                console.print(commands_config["server_nav"]["logs"]["not_exist"].format(name=name))
        # Channel Navigation
        elif check_command_start(C, "channel_nav"):
            if not client.current_guild:
                console.print(commands_config["channel_nav"]["logs"]["no_server_selected"])
                continue
            ...
            try:
                if channel:
                    ...
                    console.print(commands_config["channel_nav"]["logs"]["success"].format(name=channel.name))
                    await client.refresh_history()
                else:
                    console.print(commands_config["channel_nav"]["logs"]["not_exist"].format(name=name))
        ...
        #ISSUE:Not work
            ...
            try:
                ...
                console.print(commands_config["message_react"]["logs"]["error"].format(e=e))
            await client.refresh_history()
            continue
        #ISSUE: Not work
        # Open attachments command
            ...
            if not parts: # No arguments given, print usage
                console.print(commands_config["open_attachment"]["logs"]["usage"].format(cmdK=cmdK))
                continue
            
            try:
                msg_idx = int(parts[0]) # First argument is message number
            except ValueError:
                console.print(commands_config["open_attachment"]["logs"]["invalid_msg_index"])
                continue
            
            try:
                msg = client.history_buffer[msg_idx - 1]
            except IndexError:
                console.print(commands_config["open_attachment"]["logs"]["msg_index_out_of_range"])
                continue
            
            attachments = msg.attachments
            if not attachments:
                console.print(commands_config["open_attachment"]["logs"]["no_attachments"])
                continue
            ...
            if len(parts) > 1: 
                ...
                indices = list(range(1, len(attachments)+1)) # Open all if no specific indices given
            
            for i in indices:
                if i < 1 or i > len(attachments):
                    console.print(commands_config["open_attachment"]["logs"]["attachment_index_out_of_range"].format(index=i))
                    continue
                ...
                try:
                    ...
                    console.print(commands_config["open_attachment"]["logs"]["error_saving"].format(error_msg=e))
                    continue
                
                # Check file extension for open method.
                if attachment.filename.lower().endswith((".png", ".jpg", ".jpeg", ".gif")):
                    webbrowser.open(filepath.as_uri())
                    console.print(commands_config["open_attachment"]["logs"]["opened_image"].format(filename=attachment.filename))
                elif attachment.filename.lower().endswith(".txt", ".md", ".py", ".js"):
                    try:
                        if os.name == 'nt': # For Windows
                            os.startfile(filepath)
                        else: # For macOS/Linux
                            subprocess.run(["open", filepath]) # 'open' for macOS, 'xdg-open' for Linux (often default for 'open')
                        console.print(commands_config["open_attachment"]["logs"]["opened_text"].format(filename=attachment.filename))
                    except Exception as e:
                        console.print(commands_config["open_attachment"]["logs"]["error_opening_file"].format(error_msg=e))
                else:
                    console.print(commands_config["open_attachment"]["logs"]["no_open_command"].format(filename=attachment.filename))
            continue
        ...
        # Configuration Editor command
            ...
            for key, value in current_config.items():
                if isinstance(value, dict):
                    console.print(f"[i]Skipping complex config item: {key} (dictionary).[/i]")
                ...
                if prompt_val.strip() == "":
                    ...
                    try:
                        if isinstance(value, bool):
                            new_config[key] = prompt_val.lower() in ("true", "1", "yes", "y")
                        ...
                        console.print(f"[e]Invalid input for {key}. Keeping original value.[/e]")
                        new_config[key] = value
            ...
            if confirm.lower().startswith("y"):
                try:
                    ...
                    console.print(commands_config["config_editor"]["logs"]["error_saving"].format(error_msg=e))
            else:
        ...
        # Replying 
            ...
            parts = C[prefix_len:].split(' ', 1) # Split only once to get index and rest of message
            if len(parts) < 2:
                console.print(commands_config["reply"]["logs"]["error"].format(error_msg="Missing message number or content"))
                await client.refresh_history()
                continue
            try:
                idx = int(parts[0])
                ...
                console.print(commands_config["reply"]["logs"]["error"].format(error_msg=str(e)))
            await client.refresh_history()
            continue
            ...
            for each_ind_str in indices_str:
                try:
                    ...
                    if msg.author.id != client.user.id:
                        console.print(commands_config["delete_msg"]["logs"]["not_own_message"])
                    else:
                        await msg.delete()
                except ValueError:
                    console.print(commands_config["delete_msg"]["logs"]["invalid_index"].format(index=each_ind_str))
                except IndexError:
                    console.print(commands_config["delete_msg"]["logs"]["out_of_range"].format(index=each_ind_str))
                except Exception as e:
                    console.print(commands_config["delete_msg"]["logs"]["generic_error"].format(index_str=each_ind_str, error_msg=str(e)))
        ...
        # Message forward
            ...
            parts = C[prefix_len:].split(" ", 1) # Split into message number and rest (user arg)
            if len(parts) < 2:
            ...
            try:
                ...
                continue
            if not user_arg:
                console.print(commands_config["forward_msg"]["logs"]["user_not_specified"])
                continue
            try:
                msg = client.history_buffer[idx-1]
                u = discord.utils.get(client.users, name=user_arg)
                if not u:
                    console.print(commands_config["forward_msg"]["logs"]["no_such_user"].format(user_arg=user_arg))
                    continue
                dm = await u.create_dm()
                await dm.send(msg.content)
            except IndexError:
                console.print(commands_config["forward_msg"]["logs"]["invalid_msg_num"]) # Re-use for out of bounds
            except Exception as e:
                console.print(commands_config["forward_msg"]["logs"]["generic_error"].format(error_msg=str(e)))
            await client.refresh_history()
            continue
        ...
        # Upload staging
            ...
            for p in picked:
                ...
                console.print(commands_config["upload_stage"]["logs"]["staged"].format(filename=dst.name))
            continue
        ...
        # DM / Friend Navigation
            ...
            if user:
                ...
                console.print(commands_config["dm_nav"]["logs"]["success"].format(name=user.name))
                channel = await user.create_dm()
                client.current_channel = channel
                client.history_offset = 0
                await client.refresh_history()
            else:
                console.print(commands_config["dm_nav"]["logs"]["not_found"].format(name=name))
        ...
        # Notifications
            ...
            for i,m in enumerate(client.pending_pings,1):
                console.print(commands_config["notifications"]["logs"]["ping_details"].format(
                    index=i,
            ...
            if not idx_str.isdigit():
                console.print(commands_config["go_to_notification"]["logs"]["bad_index"])
                continue
            ...
            if client.current_channel:
                try:
                    if client.upload_staged:
                        ...
                        F = None
                    await client.current_channel.send(C, files = F) # Use C (stripped text) for sending
                    client.upload_staged.clear()
                    await client.refresh_history()
                except Exception as e:
                    console.print(commands_config["message_send"]["logs"]["fail"].format(error_msg=str(e)))
            else:
                console.print(commands_config["message_send"]["logs"]["no_channel_selected"])
