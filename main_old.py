...
CONTEXT_WINDOW_SHOW_MESSAGE_TOTAL = _["events"]["history_render"][
    "message_total"
]  # Total messages to show in a single buffer
    ...
    def get_completions(self, document: Document, complete_event):
        ...
        if text.startswith(edit_prefix):
            ...
            if parts and parts[0].isdigit():
                idx = int(parts[0])
                try:
                    orig = self.client.history_buffer[
                        len(self.client.history_buffer) - idx
                    ].content
                except Exception:
                    orig = ""
        ...
        if text.startswith(":"):
            ...
            for emoji_code in full_emoji_list:
                ...
                if core_emoji.lower().startswith(prefix.lower()):
                    char = emoji.emojize(
                        emoji_code, language="alias", variant="emoji_type"
                    )
                    display = f"{char} {emoji_code}"
                    yield Completion(
                        emoji_code, display=display, start_position=-len(prefix) - 1
                    )
    ...
    def fmt_author(self, member):
        ...
        if display_name != username:
            return _["events"]["format"]["author"].format(
                display_name=display_name, name=username
            )
        return username
...
async def start_cli(client):
    ...
    while True:
        try:
            ...
            with patch_stdout(
                sys.__stdout__
            ):  # this handles when the on_message triggers, and the input remain in the input, but somehow the color in rich moudle turns into rwa code
...
-> OR -< # Scroll to latest OR oldest
        ...
        # React to Message
            ...
            parts = C[prefix_len:].split(
                " ", 1
            )  # expecting something like: "react [message_index] [emoji]"
            # if len(parts) < 10:
        ...
        # Message forward
            ...
            try:
                ...
                console.print(
                    commands_config["edit"]["logs"]["error"].format(error_msg=str(e))
                )
            await client.refresh_history()
            continue
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    # Token mode
    parser.add_argument(
        "-t", "--token", help="Login using a raw Discord token", metavar="TOKEN"
    )
